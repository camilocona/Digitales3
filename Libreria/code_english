#ifndef FMU_MCX153_LIB_H             // Start include guard to prevent multiple inclusions
 ...
 * Flash Status Register (Flash module status register)
 */
typedef union {
...
/* Constants for possible CMDPRT field values indicating protection levels */
 ...
 * Flash Configuration Register (Flash module configuration register)
 */
typedef union {
    uint32_t WORD;                  // Full 32-bit access
    struct {
        uint32_t RESERVED0:7;       // Bits 0-6: Reserved, not used
        uint32_t CCIE:1;            // Bit 7: Command Complete Interrupt Enable
        uint32_t ERSREQ:1;          // Bit 8: Mass Erase Request (read-only)
        uint32_t RESERVED1:7;       // Bits 9-15: Reserved, not used
        uint32_t DFDIE:1;           // Bit 16: Double Bit Fault Detect Interrupt Enable
        uint32_t RESERVED2:7;       // Bits 17-23: Reserved, not used
 ...
 * Flash Control Register (Flash module control register)
 */
typedef union {
    uint32_t WORD;                  // Full 32-bit access
    struct {
        uint32_t RWSC:4;           // Bits 0-3: Read Wait-State Control
        uint32_t RESERVED0:4;      // Bits 4-7: Reserved, not used
        ...
        uint32_t RESERVED3:7;      // Bits 25-31: Reserved, not used
    } BITS;                         // Bit-field access structure
} __FMU_FCTRL_t;
...
/********************************************************************
 * 2. Declaration of FMU module structure mapping all registers
 ********************************************************************/
typedef struct {
...
/* FMU base address as defined in datasheet */
#define kFMU_BASE_ADDR 0x40095000UL

/* Macro that casts the FMU base address to a pointer to FMU registers structure */
#define sFMU (*((__FMU__t *) kFMU_BASE_ADDR))

/* Macros for direct access to full 32-bit registers */

/* Macro for FSTAT register (Flash Status) */
#define rFMU_FSTAT       sFMU.FSTAT.WORD
/* Macro for FCNFG register (Flash Configuration) */
#define rFMU_FCNFG       sFMU.FCNFG.WORD
/* Macro for FCTRL register (Flash Control) */
#define rFMU_FCTRL       sFMU.FCTRL.WORD
/* Macros for FCCOB registers (Flash Command Objects) */
