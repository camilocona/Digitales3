...
int main() {
    ...
    sleep_ms(10);  // espera a que se conecte el puerto USB
    }
    configure_i2c();
    ...
    while (1) {
        ...
        {
            ///printf("Guardando: lat=%.6f, lon=%.6f, dB=%.2f\n", GPS.Lat, GPS.Lon, 0.0f);
            struct Coordenadas datos = {GPS.Lat, GPS.Lon, 0.0f};

            ///< Guarda datos en la EEPROM con la dirección actualizada
            writeEeprom(currentAddress, datos);
            currentAddress += sizeof(struct Coordenadas);
            if (currentAddress >= 2048 ) currentAddress = 0;  ///< Reinicia a la dirección 0 cuando alcance el límite de la memoria
        ...
        if (medir_ruido_flag) {
            ...
            for (int i = 0; i < 20; i++) {
                ...
                if (GPS.Status == 1) {
                    ...
                    printf("No hay señal GPS. Muestra descartada.\n");
                }

                sleep_ms(500); // Esperar 0.5 s antes de la siguiente muestra
            }
