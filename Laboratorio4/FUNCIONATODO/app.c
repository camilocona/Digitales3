    ...
    while (1)
    {
        if (fgets(data, sizeof(data), stdin) != NULL) {
            ...
            if(strncmp(data, "START", 5) == 0) {
                ...
                bstart=true;
                gpio_put(LED_VERDE, 1); // Encender LED verde
            }
                ...
                gpio_put(LED_VERDE, 0);  // Apagar LED verde
                bstart=false;
                bstatus=false;
                bclean=false;
            } else {
...
int main() {
    ...
    gpio_put(LED_VERDE, 0); // Apagado inicial

    gpio_init(LED_ROJO);
    gpio_set_dir(LED_ROJO, GPIO_OUT);
    gpio_put(LED_ROJO, 0); // Apagado inicial


    gpio_init(LED_AMARILLO);
    gpio_set_dir(LED_AMARILLO, GPIO_OUT);
    gpio_put(LED_AMARILLO, 0); // Apagado inicial

    gpio_init(LED_NARANJA);
    gpio_set_dir(LED_NARANJA, GPIO_OUT);
    gpio_put(LED_NARANJA, 0); // Apagado inicial


    gpio_init(BOTON_OK);
    gpio_set_dir(BOTON_OK, GPIO_IN);
    gpio_pull_up(BOTON_OK); // Usamos resistencia pull-up interna
    ...
    while (1) {
        if (uart_flag) {
            ...
            if (data == '\n') {
                ...
                if(GPS.Status == 1){
                    gpio_put(LED_ROJO, 0);      // Apagar LED rojo
                    gpio_put(LED_PIN, 1);       // LED integrado (opcional)
                    bestado = true;
                    benableW = true;
                } else {
                    gpio_put(LED_ROJO, 1);      // Encender LED rojo
                    gpio_put(LED_PIN, 0);
                    bestado = false;
        ...
        if (prev_boton && !boton_actual && bstart) {  // Flanco de bajada y solo si el sistema está iniciado
        ...
        if (medir_ruido_flag) {
            ...
            gpio_put(LED_VERDE, 0);       // Apagar LED verde
            gpio_put(LED_AMARILLO, 1);    // Encender LED amarillo
            ...
            gpio_put(LED_AMARILLO, 0);    // Apagar LED amarillo
