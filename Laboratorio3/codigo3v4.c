...
void modo_interactivo() {
    ...
    while (sistema_activo) {
        ...
        if (!capturando && sistema_activo && modo_manual_activo &&
            (to_ms_since_boot(get_absolute_time()) - ultimo_envio >= 500)) {
            float rpm = medir_rpm();
            printf("PWM: %d %% | RPM: %.2f\n", pwm_actual, rpm);
