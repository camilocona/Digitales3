...
// Modo: 0 = Polling, 1 = IRQ, 2 = Combinado
int modo_medicion = 0;
...
float medir_rpm() {
    if (modo_medicion == 0) {
        ...
        uint32_t delta = ahora - last_calc;
        if (delta >= 500) {
...
void modo_interactivo() {
    ...
    while (sistema_activo) {
        ...
        if (!capturando && sistema_activo && (to_ms_since_boot(get_absolute_time()) - ultimo_envio >= 500)) {
            float rpm = medir_rpm();
            printf("PWM: %d %% | RPM: %.2f\n", pwm_actual, rpm);
            ultimo_envio = to_ms_since_boot(get_absolute_time());
        }
    }
}
...
int main() {
    ...
    printf("Comandos: START <x>, PWM <x>, STOP\n");
    modo_interactivo();
